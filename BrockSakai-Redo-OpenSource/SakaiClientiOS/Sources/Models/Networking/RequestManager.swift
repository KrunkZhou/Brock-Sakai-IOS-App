...
class RequestManager {
    ...
    private let portalURL = URL(string: "https://lms.brocku.ca/portal")
...
extension RequestManager: LoginService {
    ...
    func validateSiteStatus(onSuccess: @escaping () -> Void,
                            onFailure: @escaping (SakaiError?) -> Void) {
        let urlsites = SakaiEndpoint.sites.getEndpoint()
        makeRequestWithoutCache(url: urlsites, method: .get) { [] (data, err) in
            let decoder = JSONDecoder()
            guard let data = data else {
                onFailure(err)
                return
            ...
            do {
                let collection = try decoder.decode(SiteCollection.self, from: data)
                //self?.userId = session.userEid
                //print(collection.siteCollection.count)
                if(collection.siteCollection.count>1){
                    onSuccess()
                }else{
                    onFailure(err)
                }
